...
Upgrades the **CLI binary** itself: it checks the latest GitHub release and, if
you are behind, re-runs the install script. On Windows it prints the release URL
...
It does not touch the server image. `konet start` only pulls
